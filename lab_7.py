...
if __name__ == '__main__':
    ...
    p_i = np.array([stats.norm.cdf(limits[0])])
    n_i = np.array([len(distr[distr <= limits[0]])])
    ...
    for i in range(0, k):
        ...
        n_i = np.append(n_i, len(distr[(distr <= limits[i + 1]) & (distr >= limits[i])]))

    p_i = np.append(p_i, 1 - stats.norm.cdf(limits[-1]))
    n_i = np.append(n_i, len(distr[distr >= limits[-1]]))
